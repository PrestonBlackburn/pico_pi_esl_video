...
// #define NTP_SERVER_IP "216.239.35.0"  // time.google.com (bypass DNS)
...
int get_ntp_time(time_t *result) {
    ...
    printf("getting the ntp time\n");
    // Start the request
    if (!async_context_add_at_time_worker_in_ms(
        cyw43_arch_async_context(),  
        &state->request_worker, 
        0
    )) {
    ...
    while (!state->got_result && timeout > 0) {
        sleep_ms(100);
        timeout -= 100;
    }
    ...
    int return_code = -1;
    if (state->got_result && state->success) {
        if (result) {
            *result = state->result_time;
        }
        return_code = 0;  // Success!
    } else if (!state->got_result) {
